...
# @Time         : 18/08/2022 20:17
# @Author       : tl22089
# @File         : centralized_adapt.py
...
if __name__ == '__main__':
    ...
    log_name = '../logs/Central-adapt-{:}-{:}-{:}-{:}-{:}-{:}-model-{:}-iid-{:}-bs-{:}/'.format(now.year, now.month,
                                                                                        now.day,
                                                                                        now.hour,
                                                                                        now.minute,
                                                                                        now.second,
    ...
    global_model.load_state_dict(torch.load(mn))
    ...
    for epoch in tqdm(range(args.epochs)):
        ada_batch_loss = []
        global_model.train()
        for batch_idx, (data, labels) in enumerate(adapt_loader):
            ...
            loss.backward()
            optimizer.step()
            ada_batch_loss.append(loss.item())
        loss_avg = sum(ada_batch_loss) / len(ada_batch_loss)
    ...
    fn = '../save/centralized_adapt_{:}_{:}_{:}.h5'.format(args.signal, args.model, args.snr_high)
    mn = '../save/centralized_adapt_{:}_{:}_{:}.pt'.format(args.signal, args.model, args.snr_high)
