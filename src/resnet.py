...
class ResNet(nn.Module):
    def __init__(self, args, block, num_blocks):
        ...
        if args.type != 'metric':
            self.fc = nn.Linear(in_features=512 * block.expansion, out_features=args.bs_classes)
        else:
            self.fc = nn.Linear(in_features=512 * block.expansion, out_features=args.out_dim)
...
def ResNet18(args):
    return ResNet(args, Bottleneck, [2, 2, 2, 2])
